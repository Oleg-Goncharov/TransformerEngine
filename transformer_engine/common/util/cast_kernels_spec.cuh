...
__device__ __forceinline__ e8m0_t to_e8m0(IType amax) {
  ...
  if constexpr (std::is_same_v<IType, fp16>) {
    asm volatile("fma.rn.f32.f16 %0, %1, %2, 0.0;"
                 : "=f"(amax_fp32)
                 : "h"(reinterpret_cast<uint16_t &>(amax)), "h"(max_norm_rcp));
  } else if constexpr (std::is_same_v<IType, bf16>) {
    asm volatile("fma.rn.f32.bf16 %0, %1, %2, 0.0;"
                 : "=f"(amax_fp32)
                 : "h"(reinterpret_cast<uint16_t &>(amax)), "h"(max_norm_rcp));
  }
...
template <>
inline bool hasSpec<false, false, false, float, fp8e5m2>() {
  return is_cast_only_enabled();
}
template <>
inline bool hasSpec<false, false, false, float, fp8e4m3>() {
  return is_cast_only_enabled();
}
template <>
...
__global__ void cast_mxfp8_kernel(typename CastTraits::IType *__restrict__ input,
                                  typename CastTraits::OType *__restrict__ output,
                                  e8m0_t *__restrict__ scales_rowwise, int32_t rows, int32_t cols,
...
#pragma unroll
      ...
      if constexpr (CastTraits::_cache_rowwise_scale_in_smem) {
        ...
        scales_rowwise[coords.y * scale_stride_rowwise + coords.x / CastTraits::chunkElems] =
            biased_exponent;
      }
...
#pragma unroll
      ...
      typename CastTraits::outputUnitType *output_units =
          reinterpret_cast<typename CastTraits::outputUnitType *>(output + coords.y * cols +
                                                                  coords.x);
#pragma unroll
...
__global__ void cast_mxfp8_kernel(const __grid_constant__ CUtensorMap tensor_map_input,
                                  ...
                                  e8m0_t *__restrict__ scales_colwise, int32_t rows, int32_t cols,
                                  int32_t scale_stride_rowwise, int32_t scale_stride_colwise) {
#if (defined __CUDA_ARCH__) && (__CUDA_ARCH__ >= 1000)
  using IType2 = typename ptx::FPx2<typename CastTraits::IType>;
  using OType2 = typename ptx::FPx2<typename CastTraits::OType>;
  ...
  typename CastTraits::inputUnitType *sInputUnit =
      reinterpret_cast<typename CastTraits::inputUnitType *>(sInput);
...
#pragma unroll 1
      for (int32_t iter = 0; iter < CastTraits::iterLayout::num; iter++) {
        ...
        while (!cuda_ptx::mbarrier_try_wait_parity(&ldg_consumer[write_state.index()],
                                                   write_state.phase()));

        cuda_ptx::cp_async_bulk_tensor(
            cuda_ptx::space_shared, cuda_ptx::space_global,
            /*dstMem=*/sInput + write_state.index() * CastTraits::blockIterDim::num,
            &tensor_map_input,
            /*tensorCoords=*/{int32_t(coords.x), int32_t(coords.y)},
            &ldg_producer[write_state.index()]);
        cuda_ptx::mbarrier_arrive_expect_tx(
            cuda_ptx::sem_release, cuda_ptx::scope_cta, cuda_ptx::space_shared,
            &ldg_producer[write_state.index()],
...
#pragma unroll 1
      for (int32_t iter = 0; iter < CastTraits::numStages - 1; iter++) {
        ...
        while (!cuda_ptx::mbarrier_try_wait_parity(&stg_producer[read_state.index()],
                                                   read_state.phase()));

        cuda_ptx::cp_async_bulk_tensor(cuda_ptx::space_global, cuda_ptx::space_shared,
...
#pragma unroll 1
      for (int32_t iter = CastTraits::numStages - 1; iter < CastTraits::iterLayout::num; iter++) {
        ...
        cuda_ptx::mbarrier_arrive_expect_tx(cuda_ptx::sem_release, cuda_ptx::scope_cta,
                                            cuda_ptx::space_shared,
                                            &stg_consumer[read_state.index()], 0u);
      }
    }
...
#pragma unroll 1
    for (int32_t iter = 0; iter < CastTraits::iterLayout::num; iter++) {
      ...
      while (!cuda_ptx::mbarrier_try_wait_parity(&ldg_producer[read_state.index()],
                                                 read_state.phase()));
...
#pragma unroll
            ...
            cuda_ptx::mbarrier_arrive_expect_tx(cuda_ptx::sem_release, cuda_ptx::scope_cta,
                                                cuda_ptx::space_shared,
                                                &ldg_consumer[read_state.index()], 0u);
          }
...
#pragma unroll
     ...
     (__CUDA_ARCH_HAS_FEATURE__(SM120_ALL)))
            ...
            {
              typename CastTraits::rowOutputUnitType
                  rRowOutputUnit[CastTraits::rowNumOutUnitsPerChunk];
              typename CastTraits::rowOutputUnitType
                  rColOutputUnit[CastTraits::rowNumOutUnitsPerChunk];
...
#pragma unroll
              ...
              {
                while (!cuda_ptx::mbarrier_try_wait_parity(&stg_consumer[read_state.index()],
                                                           read_state.phase() ^ 1));
...
#pragma unroll
      ...
      cuda_ptx::mbarrier_arrive_expect_tx(cuda_ptx::sem_release, cuda_ptx::scope_cta,
                                          cuda_ptx::space_shared, &stg_producer[read_state.index()],
                                          0u);
      read_state++;
    }
...
__global__ void cast_mxfp8_kernel(const __grid_constant__ CUtensorMap tensor_map_input,
                                  const __grid_constant__ CUtensorMap tensor_map_rowwise_output,
                                  const __grid_constant__ CUtensorMap tensor_map_colwise_output,
                                  e8m0_t *scales_rowwise, e8m0_t *scales_colwise, int32_t rows,
...
#pragma unroll 1
    for (int32_t iter = 0; iter < CastTraits::numStages - 1; iter++) {
      ...
      cuda_ptx::cp_async_bulk_tensor(cuda_ptx::space_shared, cuda_ptx::space_global,
                                     sInput + iter * CastTraits::blockIterDim::num,
                                     &tensor_map_input, {int32_t(coords.x), int32_t(coords.y)},
                                     &producer[iter]);
      cuda_ptx::mbarrier_arrive_expect_tx(
          cuda_ptx::sem_release, cuda_ptx::scope_cta, cuda_ptx::space_shared, &producer[iter],
          CastTraits::blockIterDim::num * sizeof(typename CastTraits::IType));
    }
  }
#pragma unroll 1
  for (int32_t iter = 0; iter < CastTraits::iterLayout::num; iter++) {
    ...
    while (!cuda_ptx::mbarrier_try_wait_parity(&producer[states.index()], states.phase()));
...
#pragma unroll
    ...
    if (warpId == 0 && leader) {
      ...
      cuda_ptx::cp_async_bulk_tensor(
          cuda_ptx::space_global, cuda_ptx::space_shared, &tensor_map_rowwise_output,
          {int32_t(coords.x), int32_t(coords.y)}, sRowOutput + gmem_offset);
      cuda_ptx::cp_async_bulk_tensor(
          cuda_ptx::space_global, cuda_ptx::space_shared, &tensor_map_colwise_output,
          {int32_t(coords.x), int32_t(coords.y)}, sColOutput + gmem_offset);
      cuda_ptx::cp_async_bulk_commit_group();
    }
    states++;
  }
